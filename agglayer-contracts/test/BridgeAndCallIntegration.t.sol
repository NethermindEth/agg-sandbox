...
contract BridgeAndCallIntegrationTest is Test {
    ...
    event DebugClaimMessage(string msg, address destinationAddress, uint256 amount, address originAddress, uint32 originNetwork, bytes metadata);
    ...
    function testBridgeAndCallWorkflow() public {
        ...
        try bridgeL2.claimMessage(
            ...
            console2.log("Message claim succeeded!");
            
            // Check if the asset is now claimed (should be claimed by JumpPoint)
            bool isAssetClaimedAfter = bridgeL2.isClaimed(0, NETWORK_ID_L1);
            console2.log("Asset claimed status after message claim:", isAssetClaimedAfter);
            ...
            if (l2TokenAddress.code.length > 0) {
                ...
                console2.log("User1 balance after:", user1Balance);
                console2.log("User2 balance after:", user2Balance);
            } else {
                console2.log("Wrapped token not yet deployed");
            }
            ...
            console2.log("Message claim failed with low-level error");
            ...
            if (lowLevelData.length >= 4) {
                ...
                if (errorSelector == 0x37e391c3) {
                    console2.log("ERROR: MessageFailed() - This is the error we're investigating!");
                } else if (errorSelector == 0x646cf558) {
                    console2.log("ERROR: AlreadyClaimed() - Asset was already claimed");
                } else {
                    console2.log("ERROR: Unknown error selector");
                }
            }
            
            revert("Message claim failed with low-level error");
        }
