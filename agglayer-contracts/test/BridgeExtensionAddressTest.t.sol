...
contract BridgeExtensionAddressTest is Test {
    ...
    function testBridgeExtensionDifferentAddresses() public {
        ...
        assertFalse(sameAddress, "BridgeExtension addresses should be different due to different bridge constructor params");
    ...
    function testCalculateCreate2Address() public {
        ...
        bytes memory bytecode1 = abi.encodePacked(
            type(BridgeExtension).creationCode,
            abi.encode(mockBridge1)
        );
        
        bytes memory bytecode2 = abi.encodePacked(
            type(BridgeExtension).creationCode,
            abi.encode(mockBridge2)
        );
