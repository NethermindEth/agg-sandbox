...
import { filterAvailableNetworks } from './utils/network-check';
import { getChainIdForNetwork } from './utils/config';

// Initialize Web3 plugin for lxly.js
use(Web3ClientPlugin);

export class BridgeClientWrapper {
  private client: LxLyClient;
  private config: BridgeConfig;
  private providers: Record<string, HDWalletProvider> = {};

  constructor(config: BridgeConfig) {
    this.config = config;
    this.client = new LxLyClient();
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const availableNetworks = await filterAvailableNetworks(this.config.networks);
      ...
      for (const [networkId, networkConfig] of Object.entries(availableNetworks)) {
        ...
        this.providers[networkId] = new HDWalletProvider([privateKey], networkConfig.rpcUrl);
      }
      ...
      await this.client.init({
        log: true,
        network: 'testnet', // Use testnet for sandbox
  ...
  async bridgeAsset(command: BridgeAssetCommand): Promise<BridgeResponse> {
    try {
      console.log('Bridging asset:', command);

      // Get chain IDs for source and destination networks
      const destinationChainId = getChainIdForNetwork(this.config, command.destinationNetwork);
  ...
  async claimAsset(command: ClaimAssetCommand): Promise<BridgeResponse> {
    try {
  ...
  async bridgeMessage(command: BridgeMessageCommand): Promise<BridgeResponse> {
    try {
  ...
  async getNetworkStatus(networkId: number): Promise<{ connected: boolean; blockNumber?: number }> {
    try {
