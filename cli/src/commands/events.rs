...
pub async fn handle_events(
    ...
    let resolved_chain = match (network_id, chain) {
        ...
        (None, Some(chain_name)) => {
            ...
            eprintln!("⚠️  --chain parameter is deprecated. Please use --network-id instead (1=L1, 1101=L2, 1102=L3).");
            chain_name
        }
        (None, None) => {
...
fn network_id_to_chain(network_id: u64) -> Result<String> {
    match network_id {
        1 => Ok("anvil-l1".to_string()),
        1101 => Ok("anvil-l2".to_string()),
        1102 => Ok("anvil-l3".to_string()),
        137 => Ok("anvil-l3".to_string()), // Polygon mainnet maps to L3 in sandbox
        31337 => Ok("anvil-l1".to_string()), // Default Anvil network
        31338 => Ok("anvil-l2".to_string()),
        31339 => Ok("anvil-l3".to_string()),
        _ => Err(crate::error::ConfigError::invalid_value(
            "network_id",
            &network_id.to_string(),
            "Unsupported network ID for events command. Use 1 (L1), 1101 (L2), or 1102 (L3)",
        )
        .into()),
    }
}
